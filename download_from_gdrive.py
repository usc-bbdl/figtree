...
def make_weekly_ppt():
    ...
    fileName = labMeetingDate.strftime("%Y_%m_%d_LabMeeting.pptx")

    if os.path.exists(fileName):
        # Find all files in this week's lab folder (excluding agenda)
        fileNames = [
            f
            for f in listdir("figure_queue/")
            if os.path.isfile(os.path.join("figure_queue/", f))
        ]
